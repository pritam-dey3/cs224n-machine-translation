...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=self.hidden_size, bidirectional=True, dropout=dropout_rate)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1): #3
        ...
        combined_outputs = torch.stack(combined_outputs) #4
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = F.softmax(e_t, dim=1) # 1
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1),
                        enc_hiddens)
        a_t = torch.squeeze(a_t, dim=1) # 2
        U_t = torch.cat((a_t, dec_hidden), dim=1) # 3
        V_t = self.combined_output_projection(U_t) # 4
